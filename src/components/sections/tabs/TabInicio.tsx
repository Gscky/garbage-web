...
type Industria = 'Todos' | 'Retail' | 'Automotriz' | 'Industrial' | 'Corporativo'

const GALERIA: { src: string; empresa: string; industria: Industria }[] = [
  { src: '/images/gallery/Limpiapies personalizado con logo COCACOLA.jpg',         empresa: 'Coca-Cola',    industria: 'Retail' },
  { src: '/images/gallery/Limpiapies personalizado con logo la hora.JPG',           empresa: 'La Hora',      industria: 'Retail' },
  { src: '/images/gallery/personalizado con logo empresa  banco .JPG',              empresa: 'Banco',        industria: 'Retail' },
  { src: '/images/gallery/personalizado con logo empresa  azul.jpg',                empresa: 'Empresa',      industria: 'Retail' },
  { src: '/images/gallery/Limpiapies personalizado con logo valvoline.JPG',         empresa: 'Valvoline',    industria: 'Automotriz' },
  { src: '/images/gallery/Limpiapies personalizado con logo rosselot.JPG',          empresa: 'Rosselot',     industria: 'Automotriz' },
  { src: '/images/gallery/personalizado con logo empresa  derco.jpg',               empresa: 'Derco',        industria: 'Automotriz' },
  { src: '/images/gallery/Limpiapies personalizado con logo Goodgear.JPG',          empresa: 'Goodgear',     industria: 'Automotriz' },
  { src: '/images/gallery/Limpiapies personalizado con logo wurth.JPG',             empresa: 'Würth',        industria: 'Industrial' },
  { src: '/images/gallery/Limpiapies personalizado con logo ALFAMETAL.jpg',         empresa: 'Alfametal',    industria: 'Industrial' },
  { src: '/images/gallery/personalizado con logo empresa  cameron.jpg',             empresa: 'Cameron',      industria: 'Industrial' },
  { src: '/images/gallery/personalizado con logo empresa  delaval.jpg',             empresa: 'DeLaval',      industria: 'Industrial' },
  { src: '/images/gallery/personalizado con logo empresa  indesa.jpg',              empresa: 'Indesa',       industria: 'Industrial' },
  { src: '/images/gallery/personalizado con logo empresa  sinergia.JPG',            empresa: 'Sinergia',     industria: 'Corporativo' },
  { src: '/images/gallery/Limpiapies personalizado con logo empresa 2.jpg',         empresa: 'Empresa',      industria: 'Corporativo' },
  { src: '/images/gallery/Limpiapies personalizado con logo empresa 3.jpg',         empresa: 'Empresa',      industria: 'Corporativo' },
  { src: '/images/gallery/Limpiapies personalizado con logo HRA.JPG',               empresa: 'HRA',          industria: 'Corporativo' },
  { src: '/images/gallery/Limpiapies personalizado con logo csi.jpg',               empresa: 'CSI',          industria: 'Corporativo' },
]

const FILTROS: Industria[] = ['Todos', 'Retail', 'Automotriz', 'Industrial', 'Corporativo']

function GaleriaFiltrable({ onCotizar }: { onCotizar: () => void }) {
  const [filtro, setFiltro] = useState<Industria>('Todos')
  const [hoveredIdx, setHoveredIdx] = useState<number | null>(null)

  const visibles = filtro === 'Todos' ? GALERIA : GALERIA.filter((g) => g.industria === filtro)

  return (
    <section style={{ backgroundColor: '#FFFFFF', padding: 'clamp(3rem, 6vw, 5rem) clamp(1.5rem, 5vw, 5rem)' }}>
      <div style={{ maxWidth: '1280px', margin: '0 auto' }}>

        {/* Encabezado */}
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', justifyContent: 'space-between', gap: '1.5rem', marginBottom: '2rem' }}>
          <div>
            <p style={{ fontFamily: 'var(--font-dm-sans)', fontWeight: 700, fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.2em', color: '#E63000', marginBottom: '0.5rem' }}>
              Galería de trabajos
            </p>
            <h2 style={{ fontFamily: 'var(--font-playfair)', fontWeight: 700, fontSize: 'clamp(1.4rem, 2.5vw, 1.9rem)', color: '#0A1628', margin: 0 }}>
              Tu empresa, en la entrada
            </h2>
          </div>
            ...
            {FILTROS.map((f) => (
              <button
                ...
                style={{
                  ...
                  padding: '0.4rem 1rem',
                  borderRadius: '100px',
                  border: filtro === f ? '1.5px solid #0A1628' : '1.5px solid rgba(10,22,40,0.15)',
                  backgroundColor: filtro === f ? '#0A1628' : 'transparent',
                  color: filtro === f ? '#FFFFFF' : 'rgba(10,22,40,0.6)',
                  cursor: 'pointer',
        ...
        <div style={{
          ...
          {visibles.map((item, i) => (
            ...
            >
              ...
              <img
                src={item.src}
                alt={`Limpiapiés personalizado con logo ${item.empresa}`}
                loading="lazy"
                style={{
              ...
              <div style={{
                ...
                <span style={{
                  ...
                  {item.empresa}
                  <span style={{ display: 'block', fontSize: '0.65rem', fontWeight: 400, opacity: 0.75, marginTop: '1px' }}>
                    {item.industria}
                  </span>
                </span>
              </div>
        ...
        <div style={{ textAlign: 'center', marginTop: '2.5rem' }}>
          <button
            ...
            style={{
              ...
              fontSize: '0.875rem', padding: '0.8rem 2rem',
...
export default function TabInicio() {
  ...
  return (
    <div className="tab-content-enter">
      ...
      {/* ── 4. Galería filtrable ─────────────────────────────────────────── */}
      <GaleriaFiltrable onCotizar={() => setActiveTab('cotizar')} />
